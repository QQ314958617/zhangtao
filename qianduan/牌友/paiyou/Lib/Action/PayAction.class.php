...
class PayAction extends Action{
  ...
  public function dopay(){
       ...
       $alipay_config=C('alipay_config');  
        ...
        $out_trade_no =uniqid();//商户订单号 通过支付页面的表单进行传递，注意要唯一！
        $subject =$_POST['info'];  //订单名称 //必填 通过支付页面的表单进行传递
        $total_fee =0.01;   //付款金额  //必填 通过支付页面的表单进行传递
        $body =$_POST['info'] ;  //订单描述 通过支付页面的表单进行传递
        $show_url = "http://www.tfskydive.com/matrix.html";  //商品展示地址 通过支付页面的表单进行传递
...
function notify_url(){
 ...
 if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS') {
        ...
        $this->redirect(C('alipay.successpage'),array('status'=>1));//跳转到配置项中配置的支付成功页面；
    }else {
